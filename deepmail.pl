#!/usr/bin/perl -w
#################################################################
# Developed by Abusos e Auditorias do Brasil
# Updated on 12/11/2018
#
#
# HostDime Brasil - AbuseBR
#################################################################
...
print "deepmail.pl - Version: $version - HDBR Abuse & Security Brasil \n\n"; 
...
Parse the last <hours> hours worth of logs:
   ...
   # deepmail -h <hours> <user or domain>
   # deepmail --hours=<hours> <user or doman>

Parse between the last <start hours> and <end hours> marks:

   # deepmail -s <start hours> -e <end hours> <user or doman>
   # deepmail --shours=<start hours> --ehours=<end hours> <user or doman>
...
Search across all users for a reseller:
   ...
   # deepmail --reseller <reseller account>
   # deepmail -r <reseller account>

Search for the log excerpt and creation time of an email address: 

   # deepmail -c <email address> 

Some example use cases:

   # deepmail teammast
   # deepmail --hours=48 teammast
   # deepmail -s 48 -e 24 teammast
   # deepmail -h 48 -r mrowner
   # deepmail -c johndoe\@ggtt.com
...
sub mailcount {
    ...
    if ($tos && $hostname =~ /\.(dizinc|hostdime)\.com$/) {
        printtosmessage();
    }
...
An account under your control was recently found to be sending out emails at a rate faster than 350 messages/hour, which resulted in the actions indicated above. As outlined in both our terms of service and general mail policies, the maximum number of messages an account is permitted to send is 500 per hour. We will review the content of the messages being sent from your account; if they appear to be spam or otherwise violate additional dizinc rules, your account may not be eligible for unsuspension. If your actions were unintentional, the content of your messages are found not to be spam, and this incident was simply an oversight of our rules, you may have the option of upgrading to dedicated servers. Continuing this type of mail volume on shared servers, however, will not be an option. The 500 mails/hour rule was put in place for our shared servers simply because it is important to us that they are not blacklisted. High volumes of mail from any account jeopardizes that goal. We welcome your response to this issue. Should you be able to assure us this issue will not recur in the future, we may be able to reinstate this account on a shared server, or assist you in upgrading your account to a dedicated system (with the assumption your messages are not found to have violated our anti-spam policies).
